...
void lua_do_all (void) {
  ...
  while (p < pos) {
    ...
    case lq_user_info:
      ...
      break;
    case lq_history:
      tgl_do_get_history (TLS, lua_ptr[p + 1].peer_id, lua_ptr[p + 2].num, lua_ptr[p + 3].num, 0, lua_msg_list_cb, lua_ptr[p].ptr);
      p += 4;
      break;
...
struct lua_function functions[] = {
  ...
  {"send_markupmsg", lq_msg_markup, { lfp_peer, lfp_string, lfp_none }},
  ...
  {"reply_markupmsg", lq_reply_markup, { lfp_msg, lfp_string, lfp_none }},
